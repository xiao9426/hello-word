...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/xiao942622/hello-word/master/bbr.sh; then
...
Download_SSR(){
	...
	env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/xiao942622/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xiao942622/hello-word/master/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xiao942622/hello-word/master/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
		if [[ ${bit} = "x86_64" ]]; then
			wget --no-check-certificate "https://raw.githubusercontent.com/xiao942622/hello-word/master/jq-linux64" -O ${jq_file}
		else
			wget --no-check-certificate "https://raw.githubusercontent.com/xiao942622/hello-word/master/jq-linux32" -O ${jq_file}
		fi
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/xiao942622/hello-word/master/xgssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="softs"
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/xiao942622/hello-word/master/xgssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		if [[ ${yn} == [Yy] ]]; then
			if [[ $sh_new_type == "softs" ]]; then
				wget -N --no-check-certificate https://raw.githubusercontent.com/xiao942622/hello-word/master/xgssr.sh && chmod +x ssr.sh
			else
				wget -N --no-check-certificate https://raw.githubusercontent.com/xiao942622/hello-word/master/xgssr.sh && chmod +x ssr.sh
			fi
