...
Download_ocserv(){
	...
	wget "https://raw.githubusercontent.com/xiao9426/hello-word/master/ocserv-0.11.8.tar.xz"
	...
	mkdir "${conf_file}"
	wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/xiao9426/doubi/master/other/ocserv.conf"
	[[ ! -s "${conf}" ]] && echo -e "${Error} ocserv 配置文件下载失败 !" && rm -rf "${conf_file}" && exit 1
}
Service_ocserv(){
	if ! wget --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/other/ocserv_debian -O /etc/init.d/ocserv; then
...
Installation_dependency(){
	...
	if [[ ${release} = "centos" ]]; then
		...
		wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/xiao9426/doubi/master/sources/us.sources.list"
		apt-get update
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/xiao9426/doubi/master/ocserv.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -N --no-check-certificate https://raw.githubusercontent.com/xiao9426/doubi/master/ocserv.sh && chmod +x ocserv.sh
